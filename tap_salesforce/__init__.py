...
def do_discover_object(sf):
    ...
    if sf.api_type == 'BULK' and not Bulk(sf).has_permissions():
        raise SymonException('Bulk API permissions are currently disabled for this Salesforce account. Enable this setting in Salesforce and try again.', 'salesforce.BulkApiDisabled')
