        ...
        try:
            ...
            if "Failed to process query: INVALID_FIELD" in message and "No such column" in message and "on entity" in message:
                ...
                try:
                    ...
                    core_message = message[message.index("No such column"):].split(" ")
                    column = core_message[3].replace("'", '"')
                    entity = core_message[6].replace("'", '"')[:-1]
                except:
                    pass
                if column is not None and entity is not None:
                    raise SymonException(f'We can\'t find {column} column on {entity} {sf.source_type}. Review the Field Level Permissions in Salesforce and try importing your data again.', 'salesforce.InvalidField')
